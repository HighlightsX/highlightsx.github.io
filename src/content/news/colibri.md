...
An inference engine in pure C with no engine dependencies, Apache-2.0, that treats VRAM, RAM and storage as a single tiered memory hierarchy, which the README calls AI memory multitiering. GLM-5.2 at 744B, Inkling at 975B, Kimi K3 at 2.8T, DeepSeek V4 Flash at 284B and OLMoE at 7B all run today behind `coli chat`, `coli serve` and `coli web`.
...
The README's central engineering commitment is **no SLA on speed, a hard guarantee on semantics**. Insufficient fast memory may reduce speed; it must not silently redefine the model. The default policy never silently changes precision or router semantics. The rule matters because silent precision downgrades under memory pressure are how an engine produces worse output while reporting success.

The instrumentation is unusual too. A Brain page renders all 19,456 experts as a cortex, coloured by storage tier and brightened by routing heat, and an Atlas page places 13,260 characterised experts in 3-D by measured routing affinity (positions come from measurement, not a learned embedding), with 1,041 replicated specialists clustering into topics like poetry, law, Chinese and SQL.
