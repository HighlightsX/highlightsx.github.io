...
description: "A cloud-native GIS built on Tauri, MapLibre and DuckDB-WASM that ships the WhiteboxTools toolbox to WebAssembly, for desktop, mobile, browser and Jupyter."
...
The demos include Manhattan building footprints extruded and coloured by construction era with a time slider running 1850 to 2025, and planetary basemaps from OpenPlanetaryMap and USGS Astrogeology for the Moon, Mars, Mercury, Venus, the Galilean moons, Titan, Pluto and Charon, with a per-project ellipsoid so distance, area and scale are computed for the right body.
...
1,000+ tools ported to WASM is a large claim to verify, and the ones you actually need are a small subset. Check that subset specifically, in the browser, on your data.
