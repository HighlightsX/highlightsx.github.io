...
You supply one reference image of an object. The pipeline returns a `THREE.Group` factory with a real runtime hierarchy (pivots, sockets, colliders), so the result can be animated. Nothing is downloaded at runtime: the gallery of examples is generated code executing in the browser, with no mesh files behind it.
...
The project is a month old, tagged `v1.5-beta` at the start of August, and the showcase gallery shows more than a README screenshot could: every model in it is live, orbitable, and its generated source is readable next to the reference photo it came from. For this genre, that makes the claim unusually easy to check.

## How it actually works

The pipeline has a quality gate. Before any code is generated, it enumerates a `detailInventory`: the identity-defining small things in the image, such as gloss, bevels, rivets, engraved linework, contours, stains and wear. Every entry has to map to a real component or material in the plan, and generation is blocked until the inventory is complete.

This targets the usual failure of an agent doing visual work. Left alone, a model produces something that reads as the right category of object and drops the specifics that make it that particular object. A checklist the generator cannot skip converts "looks about right" into a pass/fail condition.
...
It is beta, and the version badge in the README does not match the tag on the latest release, which suggests the release process is not yet automated.
